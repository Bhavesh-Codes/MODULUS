    ...
    <div className="flex flex-col h-[calc(100vh-64px)] w-full relative">
      ...
      <div className="flex-none p-6 pb-0 flex flex-col gap-6 z-20 shrink-0">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 w-full">
          <div className="flex w-full lg:flex-1 items-center gap-4 min-w-0">
            <h1 className="hidden sm:block font-jakarta font-bold text-xl text-foreground uppercase tracking-tight shrink-0 bg-[#FFD600] px-3 py-1 rounded-[0.5rem] border-2 border-foreground shadow-[2px_2px_0_black]">My Tasks</h1>
          ...
          <div className={`flex bg-card border-[3px] border-foreground rounded-[1rem] p-1 shadow-[4px_4px_0_black] transition-opacity ${archiveMode ? 'opacity-50 pointer-events-none' : ''} shrink-0`}>
            {['List', 'Kanban', 'Calendar'].map(v => (
              <button
                ...
                className={`flex items-center gap-2 px-3 sm:px-4 py-2 font-space font-bold text-sm rounded-[0.5rem] transition-all cursor-pointer outline-none ${viewMode === v ? 'bg-[#FFD600] border-[2px] border-foreground shadow-[2px_2px_0_black] translate-y-[-1px]' : 'border-[2px] border-transparent hover:bg-background'}`}
              >
