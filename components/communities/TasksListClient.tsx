        ...
        return (
          <Card key={task.id} className="flex flex-row items-stretch p-0 overflow-hidden min-h-[120px]">
            <button
              ...
              className={cn(
                "w-20 sm:w-24 shrink-0 border-r-[2.5px] border-[#0A0A0A] flex items-center justify-center transition-all duration-200 focus:outline-none focus-visible:bg-[#F5F5F0] group",
                task.completed_by_me ? "bg-[#FFD600]" : "bg-[#F5F5F0] hover:bg-[#EBEBE6]"
              )}
            >
              <div className={cn(
                "w-10 h-10 sm:w-12 sm:h-12 border-[3px] border-[#0A0A0A] rounded-[12px] flex items-center justify-center transition-all duration-200",
                task.completed_by_me 
                  ? "bg-[#0A0A0A] shadow-none" 
                  : "bg-white shadow-[4px_4px_0px_#0A0A0A] group-hover:translate-x-[2px] group-hover:translate-y-[2px] group-hover:shadow-none"
              )}>
                <Check 
                  className={cn(
                    "w-6 h-6 sm:w-8 sm:h-8 stroke-[4px] transition-all duration-300", 
                    task.completed_by_me ? "text-[#FFD600] scale-100" : "text-[#0A0A0A]/10 scale-50 opacity-0 group-hover:opacity-100 group-hover:scale-100"
                  )} 
                />
              </div>
            </button>
            <div className="flex-1 flex flex-col justify-center gap-3.5 min-w-0 py-6 px-5 sm:px-6 bg-white">
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3">
                <h3 className="font-heading text-[22px] sm:text-[24px] font-extrabold text-[#0A0A0A] leading-[1.2] break-words">
                  {task.title}
                </h3>
                <div className="flex items-center gap-2 shrink-0">
                  {dueDateStr && (
                    <span className="shrink-0 text-xs font-medium px-3 py-1 bg-[#F5F5F0] border-[1.5px] border-[#0A0A0A] rounded-full shadow-[2px_2px_0px_#0A0A0A] whitespace-nowrap">
                      {dueDateStr}
                    </span>
                  )}
                  ...
                  {isModerator && task.status === "active" && (
                    <button
                      ...
                      title="Archive Task"
                      className="flex items-center justify-center w-8 h-8 sm:w-9 sm:h-9 bg-white border-[1.5px] border-[#0A0A0A] rounded-[8px] shadow-[2px_2px_0px_#0A0A0A] hover:bg-[#FF4D4D] hover:text-white hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none transition-all duration-150 disabled:opacity-50 disabled:cursor-not-allowed shrink-0"
                    >
                      {closingId === task.id ? (
              ...
              <div className="flex flex-col gap-1.5 mt-1">
                ...
                <span className="flex items-center gap-1.5 text-[14px] font-medium text-[#555550]">
